...
Note that the `DEBUG_ENABLED` define must be enabled at compile time to use the macros to prevent you from accidentally shipping test code. You can enable it by using the debug template for the godot-cpp.

This way when you compile with the debug template the tests will also be compiled.

I highly recommend you use the clangd LSP or the macros might be kind of awful to debug if you do something wrong. The clangd preprocessor handling is so awesome it's almost impossible to make mistakes with macros.
...
See the demo/ directory for a simple example of SFT usage. To setup the demo project first run to initialize godot-cpp `git submodule update --init --recursive`

To build the gdextension `cd demo` and then `scons target=template_debug`

Once the demo is built you will be able to open the `demo/game/project.godot` file and then run the project to see the test results in the console.

## Usage

The tests can be run from anywhere in your gdextension code, running them from the initialize function in register_types.cpp has been working for me. Some tests I run from the `_enter_tree` function of the root node in my project and only in the editor because instantiating certain things in register_types can sometimes lead to crashes if your test code needs to allocate something the engine can't allocate yet.  This will only print the output to stdout though so if you want to have your test results print in the godot editor console you'll have to run them from somewhere else in your code that gets initialized later in the startup process.
